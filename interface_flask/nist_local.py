...
class NistLocal:
    ...
    def check_files(self, input_path, files):
        ...
        if not files or not files.strip():
            ...
            files_list = self.get_files_from_folder(input_path)
    ...
    def matching_nist(self, input_path, output_path, files):
        ...
        with pyms_nist_search.Engine(
            ...
            for file in files_list:
                ...
                progress_interval = max(50, total_rows // 10)
                ...
                yield f"✨ {file}: {compounds_identified}/{compounds_processed} composés identifiés \n 💾 Fichier CSV identifié sauvegardé: {output_filepath}"

        yield f"⏱️ NIST local search completed in {time.time()-start:.2f} secondes"
