...
#     return g
def gauss_kernel(size_x, size_y, sigma_x=1.0, sigma_y=1.0, asymmetry=0.0): #TODO modif ici
    """
    Crée un noyau gaussien 2D amélioré avec possibilité d'asymétrie
    ...
    sigma_x, sigma_y : float
        Écarts-types
    asymmetry : float
        Facteur d'asymétrie pour simuler le tailing des pics
    """
    x = np.arange(-size_x, size_x + 1)
    y = np.arange(-size_y, size_y + 1)
    X, Y = np.meshgrid(x, y, indexing='ij')
    
    # Noyau gaussien de base
    kernel = np.exp(-(X**2/(2*sigma_x**2) + Y**2/(2*sigma_y**2)))
    
    # Ajout d'asymétrie (tailing)
    if asymmetry != 0:
        # Asymétrie plus prononcée dans une direction
        tail_factor = np.exp(-asymmetry * np.maximum(0, X))
        kernel *= tail_factor
...
def get_similar_spectrum(
    ...
    idx = np.concatenate(
        np.argwhere((spectrum_sim_scores >= min_similarity) &
                    (spectrum_sim_scores <= max_similarity)))
    if (len(idx) <= 0):
        return -1
...
def simulation_from_cdf_model(
        lib_path="./lib_EIB.mgf",
        scores_path='./lib_scores.json',
        min_similarity=0.0,
        max_similarity=1.1,
        min_overlap=0.7,
        max_overlap=0.99,
        intensity_range_min=20000,
        intensity_range_max=40000,
        model_filename='./data/ELO_CDF/model.cdf',
        nb_chromato=1,
        mod_time=1.25,
        new_cdf_path='./data/ELO_CDF/',
        cdf_name="new_cdf",
        noise_loc=1000.0,
        noise_scale=500.0,
        poisson_rep=0.9
        ):
    r"""Creates a new chromatogram with overlapped clusters based on a model
    chromatogram and saves it as cdf file.
    Parameters
    ----------
    lib_path : optional
    ...
    for i in range(nb_chromato):
        ...
        new_chromato_TIC, new_chromato_cube, params = (
            create_chromato_cube_with_overlapped_cluster(
                chromato.shape,
                lib_spectra,
                array_scores,
        ...
        mv = np.tile(np.linspace(
            range_min, range_max, range_max-range_min+1).astype(int), l1*l2)
        iv = np.moveaxis(new_chromato_cube, 0, -1).flatten()
        id_to_delete = np.argwhere(
            (iv == 0) & (mv != range_min) & (mv != range_max)).flatten()
        ...
        for name, dimension in ds.dimensions.items():
            if (name == 'point_number'):
                ...
                new_cdf.createDimension(name, (len(dimension)))
        # create variable from model except for mass_values, intensity_values,
        # total_intensity, mass_range_min and mass_range_max
        for name, variable in ds.variables.items():
            if (name == 'mass_values'):
                x = new_cdf.createVariable(
...
def create_random_blob(
        mu_x, mu_y,
        mu_x_range=0, mu_y_range=0,
        sigma_x_range_min=0.5, sigma_x_range_max=5,
        sigma_y_range_min=0.5, sigma_y_range_max=5):
    ...
    return [random_mu_x, random_mu_y,
            RADIUS_FACTOR * sigma_x,
            RADIUS_FACTOR * sigma_y]


def create_overlapped_cluster(
        chrom,
        mu0,
        sigma_x_range_min=2,
        sigma_x_range_max=5,
    ...
    chrom = create_and_add_peak(
        chrom, size, np.random.uniform(20000, 40000),
        [int(blob1[0]), int(blob1[1])], (blob1[2], blob1[3])
        )
    min_overlap = min_overlap
    max_overlap = max_overlap
    for i in range(random.randint(1, 2)):
        ...
        it_max = 10000
        while (it <= it_max):
            tmp_blob = create_random_blob(
                mu0[0], mu0[1], mu_x_range=20, mu_y_range=20)
            tmp_min_overlap = 1.0
            tmp_max_overlap = 0.
            ...
            if ((tmp_min_overlap >= min_overlap and
                 tmp_max_overlap <= max_overlap)):
                chrom = create_and_add_peak(
                    chrom, size, np.random.uniform(20000, 40000),
                    [int(tmp_blob[0]), int(tmp_blob[1])],
                    (tmp_blob[2], tmp_blob[3]))
...
def create_overlapped_cluster_in_cube(
        mu0, min_overlap=0.7, max_overlap=0.99):
    ...
    # for i in range(random.randint(1, 2)): #TODO modif ici
    for i in range(random.randint(3, 6)): # augmenter le nombre de blobs dans le cluster
        it = 0
        while (it <= IT_MAX):
            tmp_blob = create_random_blob(
                mu0[0], mu0[1],
                mu_x_range=20,
                mu_y_range=80,
                sigma_x_range_min=1.5,
                sigma_x_range_max=5,
                sigma_y_range_min=8,     # <-- plus de largeur verticale
                sigma_y_range_max=20
                ) #TODO modif ici 80 au lieu de 20
            tmp_min_overlap = 1.0
            tmp_max_overlap = 0.
            for j in range(len(blob_list)):
                tmp_overlap = _blob_overlap(
                    blob_list[j], tmp_blob, sigma_dim=2)
                tmp_min_overlap = min(tmp_min_overlap, tmp_overlap)
                tmp_max_overlap = max(tmp_max_overlap, tmp_overlap)
            ...
            if ((tmp_max_overlap >= min_overlap and
                 tmp_max_overlap <= max_overlap)):
                blob_list.append(tmp_blob)
...
def create_chromato_cube_with_overlapped_cluster(
        shape,
        lib_spectra,
        array_scores,
        min_overlap=0.7,
    ...
    for it1, i in enumerate(rg1):
        for it2, j in enumerate(rg2):
            # print(str(it1 + 1) + "/" + str(nb1) + " " + str(it2 + 1) + "/" + str(nb2))
            cluster, (cluster_min_overlap, cluster_max_overlap) = (
                create_overlapped_cluster_in_cube(
                    mu0=[i, j],
                    min_overlap=min_overlap,
                    max_overlap=max_overlap
                    )
            ...
            chromato_clusters.append(cluster)
            # chromato_clusters_overlap.append((cluster_min_overlap, cluster_max_overlap)) #TODO modif ici
    ...
    for cluster in chromato_clusters:
        ...
        for i, point in enumerate(cluster):
            ...
            if (i > 0):
                #random spectrum with similarity > sim_score and 
                # < max_similarity
                spectrum_id = get_similar_spectrum(
                    spectrum_index,
                    array_scores,
                    min_similarity=min_similarity,
                    max_similarity=max_similarity
                    )
                if (spectrum_id != -1 and
                        np.max(lib_spectra[spectrum_id].peaks.mz) <= 500):
            ...
            spectra.append(
                (spectrum_obj.peaks.mz, spectrum_obj.peaks.intensities)
                )
        ...
        # clusters_created_points_spectra.append(np.array(spectra)) # modif ici
        clusters_created_points_spectra.append(spectra)
    ...
    for i, created_points_spectra in enumerate(clusters_created_points_spectra):
        ...
        for j, spectrum in enumerate(created_points_spectra):
            ...
            new_iv = centroid_to_full_nominal(
                (range_min, range_max), mass_values, int_values)
    ...
    for i in range(len(clusters_created_points)):
        ...
        for j in range(len(clusters_created_points[i])):
            mu = [int(clusters_created_points[i][j][0]),
                  int(clusters_created_points[i][j][1])]
            spectrum = tmp_spectra[i][j][1]
            sigma = (clusters_created_points[i][j][2], clusters_created_points[i][j][3])
            intensity = np.random.uniform(intensity_range_min,
                                          intensity_range_max)
            chromato_cube, area = create_and_add_peak_spectrum(
                chromato_cube,
                size=size,
                intensity=intensity,
                mu=mu,
                spectrum=spectrum,
                sigma=(sigma[0] / RADIUS_FACTOR, sigma[1] / RADIUS_FACTOR))
            cluster_created_points_intensities.append(intensity)
            cluster_created_points_area.append(area)
        clusters_created_points_intensities.append(
            cluster_created_points_intensities)
        clusters_created_points_area.append(cluster_created_points_area)

    if (add_noise):
        chromato_cube = add_noise_3D(
            chromato_cube,
            noise_loc=noise_loc,
            noise_scale=noise_scale,
            poisson_rep=poisson_rep
...
#             spectrum_id=get_similar_spectrum(spectrum_index, array_scores, min_similarity=min_similarity, max_similarity=max_similarity)
#             if (id != -1):
#                 spectrum_obj=lib_spectra[spectrum_id]
