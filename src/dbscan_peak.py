...
def detection_mass_par_mass_Dog(chromato_cube,
                                chromato_obj,
                                mod_time,
                                abs_threshold=1000,
    ...
    if is_area_deconvolution:
        spec_list, area = deconvolution.construct_spectrum(quanti_all_mass, label, chromato_cube.shape[0])
    else:
        spec_list, area = None, None
        
    return coordinates, spec_list, area
...
def rt_penalty(rt_vals, rt1_delta=5, rt2_delta=0.1):
    ...
    scaled_rt = rt_vals / np.array([rt1_delta, rt2_delta])    # Compute Manhattan (L1) distance between all rows
...
def detect_peak_dog(chromato_tic,
                    ...
                    sigma_ratio,
                    overlap):
    if np.nanmax(chromato_tic) == 0:
        sigma = 0.0
    else:
        sigma = skimage.restoration.estimate_sigma(chromato_tic, channel_axis=None)
    ...
    if (multi_processing):
        ...
        with multiprocessing.Pool(processes=num_workers) as pool:
            for i, result in enumerate(pool.starmap(
                 detect_peak_dog, [(chromato_cube[mass, :, :],
                                    abs_threshold,
                                    rel_threshold,
                                    noise_factor,
                                    min_sigma, max_sigma,
